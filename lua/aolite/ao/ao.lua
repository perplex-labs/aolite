local function createAO(Handlers)
  local ao = {
    _version = "0.0.6",
    id = "",
    _module = "",
  ...
  function ao.send(msg)
    ...
    if msg.Tags then
      if isArray(msg.Tags) then
        for _, o in ipairs(msg.Tags) do
          if not _includes({ "Reference", "Data-Protocol", "From-Module", "Type" })(o) then
            table.insert(message.Tags, o)
          end
        end
      else
        for k, v in pairs(msg.Tags) do
          if not _includes({ "Reference", "Data-Protocol", "From-Module", "Type" })(k) then
            table.insert(message.Tags, { name = k, value = v })
          end
        end
      end
