...
local function G(s, a, b, c, d)
  ...
  A = (A ~ B) ~ ((A & B) << 1) -- H(A, B);
  D = D ~ A
  D = (D >> 8) | (D << 56) --ROTR64(D, 8) --R0
  C = (C ~ D) ~ ((C & D) << 1) -- H(C, D);
  B = B ~ C
  B = (B >> 19) | (B << 45) --ROTR64(B, 19) --R1
  A = (A ~ B) ~ ((A & B) << 1) -- H(A, B);
  D = D ~ A
  D = (D >> 40) | (D << 24) --ROTR64(D, 40) --R2
  C = (C ~ D) ~ ((C & D) << 1) -- H(C, D);
  B = B ~ C
  B = (B >> 63) | (B << 1) --ROTR64(B, 63) --R3
...
local function aead_decrypt(key, nonce, crypted, header, trailer)
  ...
  absorb_data(state, trailer, TRAILER_TAG)
  local tag = finalize(state, key)
  if not verify_tag(tag, ctag) then
    return nil, "auth failure"
  end
