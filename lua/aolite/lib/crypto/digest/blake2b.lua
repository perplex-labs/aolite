...
local function compress(ctx, last)
  ...
  for i = 0, 15 do -- get little-endian words
    m[i + 1] = string.unpack("<I8", ctx.b, i * 8 + 1) --copy b as a seq of u64
  end

  for i = 1, 12 do -- twelve rounds
...
local function final(ctx)
  ...
  for i = 0, ctx.outlen - 1 do
    outtbl[i + 1] = string.char((ctx.h[(i >> 3) + 1] >> (8 * (i & 7))) & 0xff)
  end
